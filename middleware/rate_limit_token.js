...
const reqLimit = process.env.MAX_WINDOW_REQUEST_COUNT_TOKEN ;// 200 req
const delay = process.env.WINDOW_LOG_INTERVAL_IN_HOURS ;// 30 мин

const redisClient = redis.createClient();
redisClient.on('error', (err) => console.log('Redis Client Error', err));

const RateLimiterToken = async (req, res, next) => {
    ...
    try {
        ...
        // извлекает записи текущего пользователя, используя токен, возвращает значение null, если запись не найдена
        const record = token;
...
module.exports = RateLimiterToken;